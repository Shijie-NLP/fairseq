...
class MultiDecoderSpeechGenerator(SpeechGenerator):
    ...
    def generate(self, model, sample, has_targ=False, **kwargs):
        ...
        prev_output_tokens_mt = (
            src_tokens.new_zeros(src_tokens.shape[0], max_tgt_len).fill_(mt_decoder.padding_idx).int()
        )  # B x T
        ...
        finalized = [
            {
                ...
                "waveform": self.get_waveform(feat[b, :l] if l > 0 else feat.new_zeros([1, raw_dim])),
