...
from fairseq.data import ConcatDataset, Dictionary, FairseqDataset, ResamplingDataset, encoders
...
class SpeechToTextDataset(FairseqDataset):
    ...
    def collater(self, samples: List[SpeechToTextDatasetItem], return_order: bool = False) -> Dict:
        ...
        if self.speaker_to_id is not None:
            speaker = (
                torch.tensor([s.speaker_id for s in samples], dtype=torch.long).index_select(0, order).view(-1, 1)
            )
...
class SpeechToTextDatasetCreator(object):
    ...
    def from_tsv(
        ...
        if is_train_split and len(datasets) > 1 and cfg.sampling_alpha != 1.0:
            ...
            datasets = [
                ResamplingDataset(d, size_ratio=r, seed=seed, epoch=epoch, replace=(r >= 1.0))
