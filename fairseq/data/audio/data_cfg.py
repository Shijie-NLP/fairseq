...
class SingleTaskConfig(object):
    ...
    def get_loss_weight(self, num_updates):
        if self.loss_weight_schedule == "fixed":
            ...
            assert self.config.get("loss_weight_decay_steps", 0) > 0, (
            ...
            loss_weight_decay_stepsize = (self.config["loss_weight_max"] - loss_weight_min) / self.config[
                "loss_weight_decay_steps"
            ]
            weight = max(
                self.config["loss_weight_max"] - loss_weight_decay_stepsize * num_updates,
                loss_weight_min,
            )
        return weight
