...
class SpeechToSpeechDataset(SpeechToTextDataset):
    ...
    def pack_units(self, input: torch.Tensor) -> torch.Tensor:
        ...
        assert input.dim() == 1
        stacked_input = input[:-1].view(-1, self.n_frames_per_step) - offset  # remove <eos>
    ...
    def collater(self, samples: List[SpeechToSpeechDatasetItem], return_order: bool = False) -> Dict:
        ...
        tgt_speakers = None
        if self.cfg.target_speaker_embed:
            tgt_speakers = _collate_frames([x.target_speaker for x in samples], is_audio_input=True).index_select(
                0, order
            )
    ...
    def collater(self, samples: List[Tuple[SpeechToSpeechDatasetItem, Dict[str, torch.Tensor]]]) -> Dict:
        ...
        for task_name, task_dataset in self.multitask_data.items():
            ...
            out["multitask"][task_name]["net_input"] = {
                "prev_output_tokens": task_target["prev_output_tokens"].index_select(0, order),
            }
...
class SpeechToSpeechDatasetCreator(object):
    ...
    def _from_list(
        ...
        tgt_audio_paths = [
            s[cls.KEY_TGT_AUDIO] if target_is_code else (audio_root / s[cls.KEY_TGT_AUDIO]).as_posix() for s in samples
        ]
