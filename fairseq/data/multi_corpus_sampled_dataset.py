...
class MultiCorpusSampledDataset(FairseqDataset):
    ...
    def ordered_indices(self):
        ...
        if self._ordered_indices is None:
            self._ordered_indices = OrderedDict(
                [(key, dataset.ordered_indices()) for key, dataset in self.datasets.items()]
            )
    ...
    def __getitem__(self, index: int):
        ...
        """
        return OrderedDict(
            [(key, dataset[self._map_index_to_dataset(key, index)]) for key, dataset in self.datasets.items()]
        )
    ...
    def num_tokens(self, index: int):
        ...
        """
        return max(
            dataset.num_tokens(self._map_index_to_dataset(key, index)) for key, dataset in self.datasets.items()
        )
