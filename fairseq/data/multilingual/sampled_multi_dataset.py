...
class SampledMultiDataset(FairseqDataset):
    ...
    def setup_sampling(self, sample_ratios, virtual_size):
        ...
        if sample_ratios is None:
            ...
            self.virtual_size = (
                virtual_size(self.datasets, self.sample_ratios) if callable(virtual_size) else virtual_size
            )
    ...
    def _establish_virtual_datasets(self):
        ...
        rng = np.random.RandomState(
            [
                int(
                    hashlib.sha1(str(self.__class__.__name__).encode("utf-8")).hexdigest(),
                    16,
                )
                % (2**32),
        ...
        logger.info(
            f"[{self.split}] Raw sizes: {str(dict(zip(self.keys, raw_sizes)))}; raw total size: {sum(raw_sizes)}"
        )
        logger.info(
