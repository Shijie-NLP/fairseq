...
from fairseq.data.encoders import tokenizer
...
class StatefulContainer:
    ...
    def add_factory(self, name: str, factory: Callable[[], Any]) -> None:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        raise AttributeError(f"Task state has no factory or value for attribute '{name}'")
...
class FairseqTask:
    ...
    def has_sharded_data(self):
        return "," in getattr(self.cfg, "data", "")
        ...
        **kwargs,
        ...
        Args:
            ...
            task_cfg (FairseqDataclass): optional task configuration stored in the checkpoint that can be used
                                         to load datasets
        """
        raise NotImplementedError("Each task must implement `load_dataset` method.")
    ...
    def dataset(self, split):
        ...
        if not isinstance(dataset, FairseqDataset):
            raise TypeError(f"Dataset '{split}' must be of type FairseqDataset.")
        return dataset
    ...
    def filter_indices_by_size(self, indices, dataset, max_positions=None, ignore_invalid_inputs=False):
        ...
        if ignored:
            if not ignore_invalid_inputs:
                raise Exception(
                    f"Size of sample #{ignored[0]} is invalid ({dataset.size(ignored[0])}) given max_positions={max_positions}."
                    " Use --skip-invalid-size-inputs-valid-test to skip."
                )
            logger.warning(
                f"{len(ignored):,} samples skipped due to invalid sizes. max_positions={max_positions}, first few: {ignored[:10]}"
            )
        return indices

    def can_reuse_epoch_itr(self, dataset) -> bool:
        ...
        *,
        ...
        Returns:
            ~fairseq.iterators.EpochBatchIterator: a batched iterator over the
                given dataset split
        """
        can_reuse = not disable_iterator_cache and not update_epoch_batch_itr and self.can_reuse_epoch_itr(dataset)
        logger.info(f"can_reuse_epoch_itr = {can_reuse}")

        if can_reuse and dataset in self.dataset_to_epoch_iter:
            logger.debug(f"Reusing EpochBatchIterator for epoch {epoch}")
            return self.dataset_to_epoch_iter[dataset]

        dataset.set_epoch(epoch)

        def make_batches(dataset, epoch):
            logger.info(f"Creating batches for epoch {epoch}")
        ...
        batch_sampler = make_batches if getattr(self.cfg, "rebuild_batches", False) else make_batches(dataset, epoch)

        iterator = iterators.EpochBatchIterator(
            dataset=dataset,
        ...
        if can_reuse:
            self.dataset_to_epoch_iter[dataset] = iterator
    ...
    def build_model(self, cfg: FairseqDataclass, from_checkpoint: bool = False):
        ...
        return quantization_utils.quantize_model_scalar(model, cfg)

    def build_criterion(self, cfg: DictConfig, from_checkpoint: bool = False):
        """
    ...
    def state_dict(self):
        return self.state.state_dict if self.state else {}
    ...
    def source_dictionary(self) -> Optional[Dictionary]:
        """Return the source :class:`~fairseq.data.Dictionary` (if applicable for this task)."""
        return None

    @property
    def target_dictionary(self) -> Optional[Dictionary]:
        """Return the target :class:`~fairseq.data.Dictionary` (if applicable for this task)."""
        return None
    ...
    def build_generator(
        ...
        search_strategy = search.resolve_search_strategy(
            tgt_dict=self.target_dictionary, args=args, prefix_allowed_tokens_fn=prefix_allowed_tokens_fn
        )
    ...
    def reduce_metrics(self, logging_outputs, criterion):
        ...
        if self_func is not base_func:
            utils.deprecation_warning("Tasks should implement reduce_metrics directly.")
            agg = self.aggregate_logging_outputs(logging_outputs, criterion)
        ...
        if ntokens:
            ...
            warnings.warn("ntokens missing from logging outputs")

        if nsentences:
            metrics.log_scalar("bsz", nsentences, priority=190, round=1)
        else:
            warnings.warn("nsentences missing from logging outputs")

        criterion.__class__.reduce_metrics(logging_outputs)

    def get_interactive_tokens_and_lengths(self, lines, encode_fn):
        tokens = [self.source_dictionary.encode_line(encode_fn(line), add_if_not_exist=False).long() for line in lines]
        lengths = [t.numel() for t in tokens]
        return tokens, lengths

    def build_dataset_for_inference(self, src_tokens: List[torch.Tensor], src_lengths: List[int], **kwargs) -> Dataset:
        raise NotImplementedError("This task does not implement build_dataset_for_inference.")

    def begin_epoch(self, epoch: int, model: torch.nn.Module) -> None:
        """Hook function called before the start of each epoch."""
        pass

    def begin_valid_epoch(self, epoch: int, model: torch.nn.Module) -> None:
