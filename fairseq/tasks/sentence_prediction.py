...
class SentencePredictionConfig(FairseqDataclass):
    ...
    shorten_method: SHORTEN_METHOD_CHOICES = field(
        default="none",
        metadata={"help": "if not none, shorten sequences that exceed tokens_per_sample"},
    )
    ...
    add_prev_output_tokens: bool = field(
        default=False,
        metadata={"help": "add prev_output_tokens to sample, used for encoder-decoder arch"},
    )
        ...
        if not self.cfg.regression_target:
            ...
            if os.path.exists(label_path):
                ...
                def parse_regression_target(i, line):
                    values = line.split()
                    assert len(values) == self.cfg.num_classes, (
                ...
                with open(label_path) as h:
                    dataset.update(
                        target=RawLabelDataset(
                            [parse_regression_target(i, line.strip()) for i, line in enumerate(h.readlines())]
                        )
                    )
