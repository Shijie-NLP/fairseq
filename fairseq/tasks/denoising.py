...
class DenoisingConfig(FairseqDataclass):
    ...
    tokens_per_sample: int = field(
        default=512,
        metadata={"help": "max number of total tokens over all segments per sample for dataset"},
    )
...
class DenoisingTask(FairseqTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        mask_whole_words = (
            get_whole_word_mask(self.cfg.bpe, self.source_dictionary) if self.cfg.mask_length != "subword" else None
        )
        ...
        return NestedDictionaryDataset(
            {
                ...
                "net_input": {
                    ...
                    "prev_output_tokens": PadDataset(prev_output_tokens, pad_idx=pad, left_pad=False),
                },
                "target": src_dataset,
            },
