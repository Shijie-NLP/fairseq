    ...
    def generate(self, models, sample, **kwargs):
        ...
        for _ in range(max_len):
            ...
            prev_output_tokens = torch.cat(
                (
                    prev_output_tokens,
                    self.pack_units(cur_pred_out.view(bsz, 1, n_frames_per_step), n_frames_per_step),
                ),
                dim=1,
            )
...
class SpeechToSpeechTask(LegacyFairseqTask):
    ...
    def build_generator(
        ...
        if not self.args.target_is_code or self.args.eval_inference:
            ...
            self.vocoder = (
                self.vocoder.cuda() if torch.cuda.is_available() and not self.args.cpu else self.vocoder.cpu()
            )
        ...
        if self.args.target_is_code:
            if self.args.n_frames_per_step == 1:
                ...
                assert getattr(args, "beam", 1) == 1 and getattr(args, "nbest", 1) == 1, (
            ...
            if has_dual_decoder:
                ...
                lang_token_ids_aux = {
                    i for s, i in self.tgt_dict_mt.indices.items() if TextTargetMultitaskData.is_lang_tag(s)
                }
    ...
    def valid_step_with_inference(self, sample, model, generator):
        if self.args.target_is_code:
            ...
            tgt_lens = (sample["target_lengths"] - 1) * self.args.n_frames_per_step  # strip <eos>
