...
class AudioClassificationTask(AudioPretrainingTask):
    ...
    def load_dataset(self, split: str, task_cfg: AudioClassificationConfig = None, **kwargs):
        ...
        if task_cfg.multi_corpus_keys is None:
            ...
            assert len(labels) == len(self.datasets[split]), (
                f"labels length ({len(labels)}) and dataset length ({len(self.datasets[split])}) do not match"
            )
            ...
            multi_corpus_sampling_weights = [
                float(val.strip()) for val in task_cfg.multi_corpus_sampling_weights.split(",")
            ]
            data_weights = []
    ...
    def train_step(self, sample, model, *args, **kwargs):
        ...
        loss, sample_size, logging_output = super().train_step(sample, model, *args, **kwargs)
    ...
    def _inference_with_metrics(self, sample, model):
        def _compute_eer(target_list, lprobs):
            ...
            fpr, tpr, thresholds = sklearn_metrics.roc_curve(y_one_hot.ravel(), lprobs.ravel())
        ...
        with torch.no_grad():
            ...
            lprobs = model.get_normalized_probs(net_output, log_probs=True).cpu().detach()
