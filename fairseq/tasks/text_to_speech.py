...
class TextToSpeechTask(SpeechToTextTask):
    ...
    def get_speaker_embeddings_path(self):
        ...
        if self.data_cfg.config.get("speaker_emb_filename") is not None:
            speaker_emb_path = op.join(self.args.data, self.data_cfg.config.get("speaker_emb_filename"))
    ...
    def get_speaker_embeddings(cls, args):
        ...
        if args.speaker_to_id is not None:
            if args.speaker_emb_path is None:
                embed_speaker = torch.nn.Embedding(len(args.speaker_to_id), args.speaker_embed_dim)
            else:
    ...
    def log_tensorboard(self, sample, hypos, num_updates, is_na_model=False):
        ...
        for b in range(len(hypos)):
            ...
            tb_writer.add_image(f"inference_sample_{b}", data, num_updates, dataformats="HWC")
            ...
            if hypos[b]["waveform"] is not None:
                ...
                tb_writer.add_audio(f"inference_targ_{b}", targ_wave, num_updates, sample_rate=self.sr)
                tb_writer.add_audio(f"inference_pred_{b}", pred_wave, num_updates, sample_rate=self.sr)
...
def plot_tts_output(
    ...
    data_2d = [x.detach().cpu().float().numpy() if isinstance(x, torch.Tensor) else x for x in data_2d]
