...
class MultilingualLanguageModelingConfig(FairseqDataclass):
    ...
    langs: str = field(
        default="",
        metadata={"help": "comma-separated list of languages (default: all directories in data path)"},
    )
    baseline_model_langs: str = field(
        default="",
        metadata={"help": "comma-separated list of languages in the baseline model (default: none)"},
    )
...
class MultilingualLanguageModelingTask(LegacyFairseqTask):
    ...
    def load_dataset(self, split: str, epoch=1, combine=False, **kwargs):
        ...
        if self.args.lang_to_offline_shard_ratio != "":
            ...
            assert os.path.exists(self.args.lang_to_offline_shard_ratio), (
        ...
        for lang_id, language in enumerate(languages):
            ...
            add_eos_for_other_targets = (
                self.args.sample_break_mode is not None and self.args.sample_break_mode != "none"
            )
        ...
        if split == self.args.train_subset:
            ...
            if lang_to_offline_shard_ratio is not None:
                ...
                for lang in languages:
                    assert lang in lang_to_offline_shard_ratio, (
            ...
            logger.info(
                ...
                {lang: "{0:.4f}".format(sample_probs[id]) for id, lang in enumerate(languages)},
            )
            ...
            logger.info(
                ...
                {lang: "{0:.2f}".format(size_ratio[id]) for id, lang in enumerate(languages)},
            )
            logger.info(
                "Actual dataset size by language: %s",
                {lang: "{0:.2f}".format(len(lang_datasets[id])) for id, lang in enumerate(languages)},
            )
            ...
            logger.info(
                ...
                {lang: "{0:.2f}".format(len(resampled_lang_datasets[id])) for id, lang in enumerate(languages)},
            )
