...
class MaskedLMConfig(FairseqDataclass):
    ...
    include_target_tokens: bool = field(
        default=False,
        metadata={"help": "include target tokens in model input. this is used for data2vec"},
    )
...
class MaskedLMTask(FairseqTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        mask_whole_words = (
            get_whole_word_mask(self.args, self.source_dictionary) if self.cfg.mask_whole_words else None
        )
