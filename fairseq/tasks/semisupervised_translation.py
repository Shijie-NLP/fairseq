...
class SemisupervisedTranslationTask(MultilingualTranslationTask):
    ...
    def __init__(self, args, dicts, training):
        ...
        if self.lambda_denoising > 0.0 or self.lambda_denoising_steps is not None:
            denoising_lang_pairs = [
                "%s-%s" % (tgt, tgt) for tgt in {lang_pair.split("-")[1] for lang_pair in args.lang_pairs}
            ]
    ...
    def load_dataset(self, split, epoch=1, **kwargs):
        ...
        self.datasets[split] = RoundRobinZipDatasets(
            OrderedDict(
                [(lang_pair, language_pair_dataset(lang_pair)) for lang_pair in src_datasets.keys()]
                + [(_get_bt_dataset_key(lang_pair), dataset) for lang_pair, dataset in backtranslate_datasets.items()]
                + [(_get_denoising_dataset_key(lang_pair), dataset) for lang_pair, dataset in noising_datasets.items()]
            ),
