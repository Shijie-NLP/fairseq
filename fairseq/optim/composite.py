...
class FairseqCompositeOptimizer(FairseqOptimizer):
    ...
    def __init__(self, cfg: CompositeOptimizerConfig, params):
        ...
        if not cfg.dynamic_groups:
            for p in params:
                ...
                if override_config is not None and bool(override_config):
                    ...
                    assert override_config is None or override_config == overrides[group], (
            ...
            for p, params in groupped_params.items():
                ...
                if override_config is not None:
                    for pp in params[1:]:
                        assert override_config == getattr(pp, "optim_overrides", None), (
        ...
        for group, group_params in groupped_params.items():
            ...
            if group in overrides and "optimizer" in overrides[group]:
                with open_dict(optimizer_config):
                    if "lr_scale" in overrides[group]["optimizer"]:
                        ...
                        optimizer_config.lr = [lr * lr_scale for lr in optimizer_config.lr]

                        if explicit_group_present:
                            logger.info(f"For group:{group}, config as well as override present for lr")

                    if (
                        "weight_decay_scale" in overrides[group]["optimizer"]
                        and "optimizer_config" in optimizer_config
                    ):
                        weight_decay_scale = overrides[group]["optimizer"]["weight_decay_scale"]
                        optimizer_config.weight_decay = optimizer_config.weight_decay * weight_decay_scale
                        if explicit_group_present:
                            logger.info(f"For group:{group}, config as well as override present for weight_decay")
            ...
            if scheduler_config is not None:
                self.lr_schedulers[group] = build_lr_scheduler(scheduler_config, self.optimizers[group])
    ...
    def get_lr(self):
        ...
        k = "default" if "default" in self.optimizers else next(iter(self.optimizers.keys()))
    ...
    def load_state_dict(self, state_dict, optimizer_overrides=None):
        ...
        for k, state in state_dict.items():
            ...
            overrides = (
                optimizer_overrides[k] if isinstance(optimizer_overrides, dict) and k in optimizer_overrides else None
            )
