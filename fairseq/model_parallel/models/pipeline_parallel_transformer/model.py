...
class PipelineParallelTransformerModel(BaseFairseqModel):
    def __init__(self, encoder, decoder, balance, devices, chunks, checkpoint):
        ...
        encoder_module_list = [encoder.embedding_layer] + list(encoder.encoder_layers) + [encoder.final_layer_norm]
        self.num_encoder_modules = len(encoder_module_list)
        decoder_module_list = [decoder.embedding_layer] + list(decoder.decoder_layers) + [decoder.decoder_output_layer]
        ...
        self.encoder_max_positions = self.max_positions_helper(encoder.embedding_layer, "max_source_positions")
        self.decoder_max_positions = self.max_positions_helper(decoder.embedding_layer, "max_target_positions")
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens):
        if self.training:
            ...
            assert self.encoder is not None and self.decoder is not None, (
                "encoder and decoder need to be initialized by " + "calling the `prepare_for_inference_()` method"
            )
    ...
    def build_model_base(cls, args, task):
        ...
        def build_embedding(dictionary, embed_dim, path=None, num_embed_chunks=1):
            ...
            assert path is None or num_embed_chunks == 1, (
                "Loading embedding from a path with number of embedding chunks > 1" + " is not yet supported"
            )
        ...
        if args.share_all_embeddings:
            ...
            assert args.share_decoder_input_output_embed or num_embed_chunks == 1, (
                "Not sharing decoder I/O embeddings is not yet supported with number of " + "embedding chunks > 1"
            )
            encoder_embed_tokens = build_embedding(
                src_dict,
                args.encoder_embed_dim,
...
@register_model_architecture("pipeline_parallel_transformer", "transformer_iwslt_de_en_pipeline_parallel")
def transformer_iwslt_de_en_dist(args):
    transformer_iwslt_de_en(args)


@register_model_architecture("pipeline_parallel_transformer", "transformer_wmt_en_de_big_pipeline_parallel")
