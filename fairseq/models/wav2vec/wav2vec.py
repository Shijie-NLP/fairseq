...
class Wav2VecConfig(FairseqDataclass):
    ...
    sample_distance: Optional[int] = field(
        default=None,
        metadata={"help": "sample distance from target. does not work properly with cross-sampling"},
    )
    ...
    conv_aggregator_layers: str = field(
        ...
        metadata={"help": "convolutional aggregator layers [(dim, kernel_size, stride), ...]"},
    )
    ...
    project_features: PROJECT_FEATURES_CHOICES = field(
        default="none",
        metadata={"help": "if not none, features are projected using the (same or new) aggregator"},
    )
    ...
    vq_dim: int = field(
        default=0,
        metadata={"help": "uses this dimensionality for quantized vectors. 0 to use model dim // groups"},
    )
...
class Wav2VecPredictionsModel(nn.Module):
    ...
    def sample_negatives(self, y):
        ...
        negs = negs.view(fsz, bsz, self.n_negatives + self.cross_sample_negatives, tsz).permute(
            2, 1, 0, 3
        )  # to NxBxCxT
