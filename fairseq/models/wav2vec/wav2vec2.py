...
class Wav2Vec2Config(FairseqDataclass):
    ...
    quantizer_factor: int = field(
        default=3,
        metadata={"help": "dimensionality increase for inner quantizer layers (if depth > 1)"},
    )
    ...
    def __init__(self, args: Wav2Vec2Config, skip_pos_conv: bool = False, override_encoder_layer: int = None):
        ...
        if pos_conv_depth > 1:
            ...
            self.pos_conv = make_conv_pos(
                ...
                is_batch_norm=args.conv_pos_batch_norm if hasattr(args, "conv_pos_batch_norm") else False,
            )
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                ...
                if (corpus_key is None) or (
                    not isinstance(layer_check, (TransformerSentenceEncoderWithAdapterLayer,))
                ):
