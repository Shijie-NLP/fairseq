...
from fairseq.models.wav2vec.wav2vec2 import LAYER_TYPE_CHOICES, MASKING_DISTRIBUTION_CHOICES
...
class Wav2Vec2AsrConfig(FairseqDataclass):
    ...
    attention_dropout: float = field(
        default=0.0,
        metadata={"help": "dropout probability for attention weights inside wav2vec 2.0 model"},
    )
    activation_dropout: float = field(
        default=0.0,
        metadata={"help": "dropout probability after activation in FFN inside wav2vec 2.0 model"},
    )
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(
        ...
        self.layers.extend(
            [TransformerDecoderLayer(transformer_cfg, no_encoder_attn) for _ in range(transformer_cfg.decoder_layers)]
        )
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None, **unused):
        ...
        if type(prev_output_tokens) is list:
