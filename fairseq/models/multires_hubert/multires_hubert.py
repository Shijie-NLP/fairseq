...
class MultiresHubertConfig(FairseqDataclass):
    ...
    encoder_layers: int = field(
        default="2",
        metadata={"help": "num encoder layers in the each block (one sub module of the U-net)"},
    )
    override_encoder_layers: str = field(
        default="",
        metadata={"help": "specific layer numbers for each block (one sub module of the U-net) for the training"},
    )
    ...
    conv_adapator_kernal: int = field(default=7, metadata={"help": "kernal size for conv adaptor"})
    use_plain_updownsample: bool = field(default=False, metadata={"help": "whether to use plain up downsample"})
    ...
    use_single_target: bool = field(
        default=False,
        metadata={"help": "whether to use single data (in that case, we will compute with the fixed label rate)"},
    )
    use_single_prediction: bool = field(
        default=False,
        metadata={"help": "if true, we will not conduct mlm prediction in low resolution in the middle"},
    )
...
class MultiresHubertModel(BaseFairseqModel):
    def __init__(
        ...
        assert cfg.label_rate_ratios != "None", "without ratios, the model is exactly as the Hubert model"
        ...
        if cfg.override_encoder_layers != "":
            ...
            assert len(self.override_encoder_layers) % 2 == 1, (
                "must be odd number of layers if specify detailed layers"
            )
            assert len(self.override_encoder_layers) // 2 == len(cfg.label_rate_ratios) // 2, (
        ...
        for i in range(len(raw_label_rate_ratios) // 2):
            ...
            if self.override_encoder_layers is not None:
                ...
                override_decoder_layer = self.override_encoder_layers[self.len_encoder_modules - 1 - i]
            else:
                override_encoder_layer, override_decoder_layer = None, None

            self.label_rate_ratios.append((raw_label_rate_ratios[i * 2], raw_label_rate_ratios[i * 2 + 1]))
            ...
            if not first_pos_conv:
                self.encoders.append(
                    TransformerEncoder(cfg, override_encoder_layer=override_encoder_layer)
        ...
        self.label_nums = len(self.feature_ds_rates)  # the number of labels for prediction (activate at iter 2)

        if type(self.label_rates) is float:
        ...
        if cfg.target_glu:
            ...
            for i in range(self.predictor_head_num):
                self.target_glus.append(nn.Sequential(nn.Linear(final_dim, final_dim * 2), nn.GLU()))
        ...
        for i in range(self.predictor_head_num):
            ...
            self.label_embs_concat.append(nn.Parameter(torch.FloatTensor(num_classes, final_dim)))
    ...
    @classmethod
    def build_model(cls, cfg: MultiresHubertConfig, task: MultiresHubertPretrainingTask):
    ...
    def forward(
        ...
        def align_size_sum(feat1, pad1, feat2):
            assert abs(feat1.size(1) - feat2.size(1)) < 10, (
                "misaligned results for feat1 and feat2 of size {} - {}".format(feat1.size(1), feat2.size(1))
            )
        ...
        for i in range(self.label_nums - 1):
            x, padding_mask, mask_indices = self.upsample_modules[self.label_nums - 2 - i](
                x, padding=padding_mask, mask_indices=mask_indices
            )
        ...
        if target_list is not None:
            ...
            for i in range(self.label_nums):
                ...
                if padding_masks[i] is not None:
                    padding_masks[i] = self.forward_padding_mask(res_outputs[i], padding_masks[i])
                if multi_mask_indices[i] is not None:
                    multi_mask_indices[i] = self.forward_padding_mask(res_outputs[i], multi_mask_indices[i])
        ...
        for j in range(self.label_nums):
            ...
            label_embs_list = self.label_embs_concat[j].split([self.multires_classes[j]], 0)
            ...
            if not self.skip_masked:
                ...
                logit_m_list.append(compute_pred(proj_x_m, target[masked_indices], label_embs_list[0]))
            else:
            ...
            if not self.skip_nomask:
                ...
                logit_u_list.append(compute_pred(proj_x_u, target[nomask_indices], label_embs_list[0]))
            else:
...
class ConvAdapter(nn.Module):
    ...
    def __init__(
        ...
        def downsample_block(channel, k, stride):
            return nn.Sequential(
                ...
                norm_block(is_layer_norm=False, dim=channel, affine=not non_affine_group_norm),
                activation,
            )
    ...
    def forward(self, x, padding=None, mask_indices=None):
        ...
        if self.skip_connections:
            residual_upsample = torch.repeat_interleave(residual_before_upsample, self.upsample_rate, dim=2)
            upsample_size = min(upsample_size, residual_upsample.size(2))
            x = (x[..., :upsample_size] + residual_upsample[..., :upsample_size]) * self.residual_scale
        ...
        if self.skip_connections:
            residual_downsample = residual_before_downsample[..., :: self.downsample_rate]
            downsample_size = min(x.size(2), residual_downsample.size(2))
            x = (x[..., :downsample_size] + residual_downsample[..., :downsample_size]) * self.residual_scale

        if self.highway:
            residual_after_sample = residual_upsample[..., :: self.downsample_rate]
            final_size = min(x.size(2), residual_after_sample.size(2))
            x = (x[..., :final_size] + residual_after_sample[..., :final_size]) * self.residual_scale
        ...
        if mask_indices is not None:
            mask_indices = torch.repeat_interleave(mask_indices, self.upsample_rate, dim=1)
