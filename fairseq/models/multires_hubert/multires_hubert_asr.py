...
class MultiresHubertAsrConfig(FairseqDataclass):
    multires_hubert_path: str = field(default=MISSING, metadata={"help": "path to multires_hubert model"})
    ...
    attention_dropout: float = field(
        default=0.0,
        metadata={"help": "dropout probability for attention weights inside hubert model"},
    )
    activation_dropout: float = field(
        default=0.0,
        metadata={"help": "dropout probability after activation in FFN inside hubert model"},
    )
...
class MultiresHubertCtc(BaseFairseqModel):
    def __init__(self, cfg: MultiresHubertAsrConfig, multireshubert_encoder: BaseFairseqModel):
        super().__init__()
        self.cfg = cfg
...
class MultiresHubertEncoder(FairseqEncoder):
    def __init__(self, cfg: MultiresHubertAsrConfig, task):
        ...
        if cfg.multires_hubert_args is None:
            state = checkpoint_utils.load_checkpoint_to_cpu(cfg.multires_hubert_path, arg_overrides)
            ...
            if isinstance(multires_hubert_args, Namespace):
                cfg.multires_hubert_args = multires_hubert_args = convert_namespace_to_omegaconf(multires_hubert_args)
        ...
        model = pretrain_task.build_model(multires_hubert_args.model, from_checkpoint=True)
    ...
    def forward(self, source, padding_mask, tbc=True, **kwargs):
        ...
        with torch.no_grad() if not ft else contextlib.ExitStack():
            x, padding_mask = self.multires_hubert_model.extract_features(**multires_hubert_args)
