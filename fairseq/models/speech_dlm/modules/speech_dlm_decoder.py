...
class CrossChannelTransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, channels, no_encoder_attn=False):
        ...
        self.project_in_dim = (
            nn.Linear(input_embed_dim, embed_dim, bias=False) if embed_dim != input_embed_dim else None
        )
        ...
        self.project_out_dim = (
            nn.Linear(embed_dim, self.output_embed_dim, bias=False) if embed_dim != self.output_embed_dim else None
        )
        ...
        self.output_duration_prediction = (
            ...
            else nn.ModuleList([nn.Linear(self.output_embed_dim, 1) for _ in range(self.n_output_projections)])
        )
        ...
        for i, channel in enumerate(self.channels):
            ...
            if self.embed_positions is not None:
                positions = self.embed_positions(
                    ...
                    incremental_state=incremental_state[i] if incremental_state is not None else None,
                )
            ...
            for pred_channel in logits_dict[channel]:
                ...
                if isinstance(logits_dict[channel][pred_channel], dict):
                    out_dict[channel][pred_channel] = {
                        "pred_token": out,
                        "pred_duration": logits_dict[channel][pred_channel]["pred_duration"].float(),
