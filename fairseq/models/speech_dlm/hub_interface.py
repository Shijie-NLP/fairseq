...
class MultichannelGeneratorHubInterface(GeneratorHubInterface):
    ...
    def sample(self, sentences: List[Dict[str, str]], beam: int = 1, verbose: bool = False, **kwargs) -> List[str]:
    ...
    def score(self, sentences: List[Dict[str, str]], **kwargs):
        raise NotImplementedError("MultichannelGeneratorHubInterface doesn't support score() method")
    ...
    def generate(
        ...
        **kwargs,
        ...
        if isinstance(tokenized_sentences, dict):
            return self.generate([tokenized_sentences], beam=beam, verbose=verbose, **kwargs)[0]
        ...
        for batch in tqdm(self._build_batches(tokenized_sentences, skip_invalid_size_inputs)):
            ...
            for id, hypos in zip(batch["id"].tolist(), translations):
                ...
                for i in range(len(hypos)):
                    hypos[i]["tokens"] = {
                        channel: hypos[i]["tokens"][..., j] for j, channel in enumerate(self.channels)
                    }
        ...
        if verbose:
            ...
            for source_tokens, target_hypotheses in zip(tokenized_sentences, outputs):
                src_str_with_unk = {channel: self.string(source_tokens[channel], channel) for channel in source_tokens}
    ...
    def encode(self, sentence: Dict[str, str]) -> Dict[str, torch.LongTensor]:
        assert isinstance(sentence, dict), "Input sentence is expected to be a dictionary over channels"
    ...
    def decode(self, tokens: Dict[str, torch.LongTensor]) -> Dict[str, str]:
        assert isinstance(tokens, dict), "Input tokens are expected to be a dictionary over channels"
