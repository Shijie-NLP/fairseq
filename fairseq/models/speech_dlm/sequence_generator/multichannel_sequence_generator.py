...
class MultichannelSequenceGenerator(nn.Module):
    def __init__(
        ...
        self.duration_prediction = bool(
            str(getattr(models[0].decoder.args, "duration_prediction", "false")).lower() == "true"
        )
        self.delayed_duration = bool(
            str(getattr(models[0].decoder.args, "delayed_duration_target", "false")).lower() == "true"
        )
    ...
    def _generate(
        ...
        tokens = (
            torch.zeros(bsz * beam_size, max_len + 2, self.n_channels).to(src_tokens).long().fill_(self.pad)
        )  # +2 for eos and pad
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if not self.duration_prediction:
                ...
                dur_preds = (
                    torch.stack([net_output["pred_duration"] for net_output in lprobs_dict.values()]).squeeze(-1).T
                )
            ...
            for i in range(self.n_channels):
                ...
                if prefix_tokens is not None and step < prefix_tokens.size(1) and step < max_len:
                    ...
                    if self.duration_prediction:
                        ...
                        can_copy_mask = (
                            prefix_tokens[:, step, i].eq(self.pad) | prefix_tokens[:, step, i].eq(self.unk)
            ...
            if eos_bbsz_idx.numel() > 0:
                eos_scores = torch.stack(
                    [
                        torch.masked_select(cand_scores[:, :beam_size, i], mask=eos_mask[:, :beam_size])
                        for i in range(self.n_channels)
                    ],
                    dim=-1,
                )
            ...
            if self.is_speech_dlm:
                if self.is_duration_prediction:
                    decoder_out_divided_by_temperature = {
                        channel_src: {
                            channel_pred: {
                                "pred_token": decoder_out[0][channel_src][channel_pred]["pred_token"][:, -1:, :].div_(
                                    temperature[channel_pred]
                                ),
                    ...
                    decoder_out_divided_by_temperature = {
                        channel_src: {
                            channel_pred: decoder_out[0][channel_src][channel_pred][:, -1:, :].div_(
                                temperature[channel_pred]
                            )
                            for channel_pred in decoder_out[0][channel_src]
                        }
