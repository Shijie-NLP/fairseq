...
class TransformerFromPretrainedXLMModel(TransformerModel):
    ...
    def build_model(self, args, task, cls_dictionary=MaskedLMDictionary):
        assert hasattr(args, "pretrained_xlm_checkpoint"), (
            "You must specify a path for --pretrained-xlm-checkpoint to use --arch transformer_from_pretrained_xlm"
        )
        ...
        assert not (getattr(args, "init_encoder_only", False) and getattr(args, "init_decoder_only", False)), (
        ...
        for search_key in ["embed_tokens", "embed_positions", "layers"]:
            if search_key in key:
                ...
                assert subkey in state_dict, (
                    ...
                    "{} into Transformer.".format(str(state_dict.keys()), subkey, key, pretrained_xlm_checkpoint)
                )
...
class TransformerEncoderFromPretrainedXLM(TransformerEncoder):
    def __init__(self, args, dictionary, embed_tokens):
        ...
        assert hasattr(args, "pretrained_xlm_checkpoint"), (
            "--pretrained-xlm-checkpoint must be specified to load Transformer encoder from pretrained XLM"
        )
...
class TransformerDecoderFromPretrainedXLM(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if getattr(args, "init_encoder_only", False):
            ...
            return
        assert hasattr(args, "pretrained_xlm_checkpoint"), (
            "--pretrained-xlm-checkpoint must be specified to load Transformer decoder from pretrained XLM"
        )
