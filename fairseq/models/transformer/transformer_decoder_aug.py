...
class AugTransformerDecoderBase(TransformerDecoderBase):
    ...
    def __init__(
        ...
        self.layers.extend(
            [self.build_decoder_layer(cfg, encoder_attn_merge_type, dropnet_ratio) for _ in range(cfg.decoder.layers)]
        )
...
class AugTransformerDecoder(AugTransformerDecoderBase):
    def __init__(
        ...
        super().__init__(
            ...
            no_encoder_attn=False,
            output_projection=output_projection,
            encoder_attn_merge_type=getattr(args, "synthesizer_augmented_cross_attention_merge_type", "sequential"),
