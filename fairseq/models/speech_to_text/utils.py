...
def assert_equal(value1, value2, name1=None, name2=None):
    ...
    if value1 != value2:
        ...
        raise ValueError("Expected {}{} == {}{}".format(str_name1, str_value1, str_name2, str_value2))
...
def lengths_to_attention_mask(
    ...
    indices = torch.arange(max_length, device=lengths.device, dtype=lengths.dtype).expand(
        max_length, max_length
    ) - torch.arange(max_length, device=lengths.device).view(max_length, -1)
    ...
    #     [True, True, True, True, True],
    # ])
    bool_mask = torch.tensor([True]).to(device=lengths.device).expand(max_length, max_length)
...
def infer_channels_from_layout(layout, channels):
    ...
    if layout in ("TBD", "BTD"):
        if channels is not None and channels != 1:
            raise ValueError("Expected channels ({}) to be 1 for layout = {}".format(channels, layout))
        if channels is None:
            return 1
    return channels
...
def lengths_to_encoder_padding_mask(lengths, batch_first: bool = False):
    ...
    bsz = lengths.size(0)
    encoder_padding_mask = torch.arange(max_lengths).to(  # a (T, ) tensor with [0, ..., T-1]
        lengths.device
