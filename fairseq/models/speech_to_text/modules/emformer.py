...
class Fp32LayerNorm(nn.Module):
    ...
    def forward(self, input):
        output = torch.nn.functional.layer_norm(
            ...
            self.torch_module.weight.float() if self.torch_module.weight is not None else None,
            self.torch_module.bias.float() if self.torch_module.bias is not None else None,
...
class NoSegAugmentedMemoryTransformerEncoderLayer(FairseqEncoder):
    ...
    def _get_attention_mask(self, input, past_length=0, left_context_cache=0):
        ...
        if self.use_mem:
            ...
            attention_mask = (1 - torch.cat([torch.cat(rc_mask, dim=0), torch.cat(query_mask, dim=0)], dim=0)).to(
                torch.bool
            )
