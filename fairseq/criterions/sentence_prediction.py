...
class SentencePredictionCriterion(FairseqCriterion):
    def __init__(self, cfg: SentencePredictionConfig, task):
        ...
        self.keep_pred_and_targ = cfg.report_mcc or cfg.report_acc_and_f1 or cfg.report_pearson_and_spearman
    ...
    def forward(self, model, sample, reduce=True):
        ...
        """
        assert (
            hasattr(model, "classification_heads") and self.classification_head_name in model.classification_heads
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        pred = np.array(list(chain.from_iterable(log.get("pred", []) for log in logging_outputs)))
        targ = np.array(list(chain.from_iterable(log.get("targ", []) for log in logging_outputs)))
        if len(pred):
