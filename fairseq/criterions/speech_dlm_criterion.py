...
class SpeechDLMCriterionConfig(FairseqDataclass):
    ...
    main_and_cross_weights: Optional[str] = field(
        default="1,0",
        metadata={
            "help": "Comma-separated list of weights of Main-channel vs Cross-channel Prediction Losses(default: 1,0)"
        },
    )
    general_unit_loss_weight: float = field(
        default=0,
        metadata={"help": "The weight of the General Prediction Loss (Next-step Unit Prediction Loss)(default: 0)"},
    )
    ...
    duration_loss_weight: float = field(
        default=1,
        metadata={"help": "The weight of the Edge Unit Duration Prediction Loss(default: 1)"},
    )
...
class SpeechDLMCriterion(FairseqCriterion):
    ...
    def __init__(
        ...
        self.channel_weights = dict(zip(self.channels, task.channel_weights))

        self.target_weights = {}
        for t in self.targets:
            if t == "next":
                self.target_weights[t] = general_unit_loss_weight
                assert general_unit_loss_weight > 0, (
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        target_prefixes = {item[:-5].split("]")[-1] for item in logging_keys if item.endswith("_loss")}
        for channel_prefix in channels:
            for target_prefix in target_prefixes:
                ...
                if "duration" not in target_prefix:
                    ...
                    metrics.log_derived(
                        "{}_ppl".format(prefix),
                        lambda meters, prefix=prefix: utils.get_perplexity(meters["{}_loss".format(prefix)].avg),
                    )
                else:
